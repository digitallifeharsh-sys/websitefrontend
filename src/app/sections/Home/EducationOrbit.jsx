...
const EducationCard = ({ title, radius, speed, angleOffset }) => {
  ...
  useFrame((state) => {
    ...
    if (htmlRef.current) {
      ...
      htmlRef.current.style.opacity = String(0.3 + depth * 0.7);
      htmlRef.current.style.zIndex = String(Math.round(depth * 1000));
  ...
  return (
    <group ref={groupRef}>
      ...
      <Html center sprite zIndexRange={[1000, 0]}>
        <div
          ref={htmlRef}
          className="rounded-xl border border-white/40 bg-white/70 px-5 py-2.5 font-semibold tracking-tight text-[#0F172A] shadow-xl shadow-indigo-500/5 backdrop-blur-md transition-colors duration-500 will-change-transform"
        >
          {title}
        </div>
      </Html>
...
const Scene = () => {
  ...
  useFrame((state) => {
    ...
    const targetX = state.pointer.y * 0.1 + 0.15;
      ...
      <group>
        ...
        <Html center className="pointer-events-none">
          <div className="flex flex-col items-center justify-center">
            <span className="text-[10px] font-extrabold tracking-widest text-[#3730A3] opacity-80">
              DNS
            </span>
            <span className="text-[8px] font-bold tracking-wider text-[#4F46E5] opacity-60">
              CORE
            </span>
          </div>
        </Html>
